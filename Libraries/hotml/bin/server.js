...
hotml_server_Main.prototype = {
	onConnect: function(ws) {
		...
		ws.on("close",function(err) {
			console.log("Sources/hotml/server/Main.hx:51:","Client disconnected");
	...
	,reload: function() {
		...
		if(this.file == null) {
			...
			console.log("Sources/hotml/server/Main.hx:69:","" + this.buildDir + "/" + this.scriptName + " cached");
			console.log("Sources/hotml/server/Main.hx:70:","Js type: " + this.file.getJsType());
			return;
		}
		var newFile = new hotml_server_Parser(data);
		var arr = this.file.makeDiffTo(newFile);
		if(hotml_server_Main.logResult) {
			console.log("Sources/hotml/server/Main.hx:75:",arr);
		}
		this.broadcast(arr);
		this.file = newFile;
	}
...
hotml_server_Parser.prototype = {
	...
	,parseRegular: function(line) {
		...
		if(this.matchObj.match(line)) {
			var name2 = this.matchObj.matched(1);
			this.setObj(name2);
			this.setNameId(name2,name2);
			return;
		}
